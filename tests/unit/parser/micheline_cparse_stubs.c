...
CAMLprim value
micheline_cparse_capture_name(value mlstate)
{
    CAMLparam1(mlstate);
    CAMLlocal1(r);
...
CAMLprim value
micheline_cparse_init(value size)
{
    CAMLparam1(size);
    CAMLlocal1(r);
...
CAMLprim value
micheline_cparse_step(value mlstate, value input, value output)
{
    ...
    default:
        ...
        snprintf(err, sizeof(err),
                 "micheline_cparse_step: unknown error code %d",
                 state->errno);
        caml_failwith(err);
    }
    CAMLreturn(r);
...
CAMLprim value
operation_cparse_step(value mlstate, value input, value output)
{
    CAMLparam3(mlstate, input, output);
    CAMLlocal3(ibuf, obuf, r);
    ibuf     = Field(input, 0);
    ...
    default:
        char err[100];
        snprintf(err, sizeof(err),
                 "operation_cparse_step: unknown error code %d",
                 state->errno);
        caml_failwith(err);
    }
    CAMLreturn(r);
