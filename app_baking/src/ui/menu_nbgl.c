...
ui_menu_main(void)
{
// This parameter shall be set to false if the settings page contains only
// information about the application (version , developer name, ...). It shall
// be set to true if the settings page also contains user configurable
...
enum {
    DUMMY_SWITCH_1_TOKEN = FIRST_USER_TOKEN,
    DUMMY_SWITCH_2_TOKEN
};
enum {
    DUMMY_SWITCH_1_ID = 0,
    DUMMY_SWITCH_2_ID,
...
static bool
nav_callback(uint8_t page, nbgl_pageContent_t *content)
{
    UNUSED(page);

    // the first settings page contains only the version and the developer
    // name of the app (shall be always on the first setting page)
    if (page == 0) {
    ...
    // the second settings page contains 2 toggle setting switches
        ...
        switches[DUMMY_SWITCH_1_ID].initState
            = (nbgl_state_t)N_storage.dummy1_allowed;
        switches[DUMMY_SWITCH_1_ID].text = "Dummy 1";
        switches[DUMMY_SWITCH_1_ID].subText
            = "Allow dummy 1\nin transactions";
        switches[DUMMY_SWITCH_1_ID].token  = DUMMY_SWITCH_1_TOKEN;
        switches[DUMMY_SWITCH_1_ID].tuneId = TUNE_TAP_CASUAL;

        switches[DUMMY_SWITCH_2_ID].initState
            = (nbgl_state_t)N_storage.dummy2_allowed;
        switches[DUMMY_SWITCH_2_ID].text = "Dummy 2";
        switches[DUMMY_SWITCH_2_ID].subText
            = "Allow dummy 2\nin transactions";
...
static void
review_warning_choice(bool confirm)
{
...
static void
controls_callback(int token, uint8_t index)
{
    UNUSED(index);
    uint8_t switch_value;
    if (token == DUMMY_SWITCH_1_TOKEN) {
        ...
        if (!N_storage.dummy2_allowed) {
            ...
            nbgl_useCaseChoice(
                &C_warning64px, "Dummy 2",
                "Are you sure to\nallow dummy 2\nin transactions?",
                "I understand, confirm", "Cancel", review_warning_choice);
        } else {
            // toggle the switch value
            switch_value = !N_storage.dummy2_allowed;
            // store the new setting value in NVM
...
#define DISABLE_SUB_SETTINGS (false)
    nbgl_useCaseSettings(APPNAME, INIT_SETTINGS_PAGE, TOTAL_SETTINGS_PAGE,
                         DISABLE_SUB_SETTINGS, ui_menu_main, nav_callback,
                         controls_callback);
}
