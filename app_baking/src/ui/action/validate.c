...
static int
crypto_sign_message(void)
{
    uint32_t info    = 0;
    size_t   sig_len = sizeof(G_context.tx_info.signature);

    cx_err_t error = bip32_derive_ecdsa_sign_hash_256(
        CX_CURVE_256K1, G_context.bip32_path, G_context.bip32_path_len,
        CX_RND_RFC6979 | CX_LAST, CX_SHA256, G_context.tx_info.m_hash,
...
void
validate_transaction(bool choice)
{
    if (choice) {
