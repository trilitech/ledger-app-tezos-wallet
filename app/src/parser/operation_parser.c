...
const tz_operation_descriptor tz_operation_descriptors[] = {
    ...
    {TZ_OPERATION_TAG_REG_GLB_CST,  "Register global constant",
     reg_glb_cst_fields                                                              },
    {TZ_OPERATION_TAG_SET_DEPOSIT,  "Set deposit limit",          set_deposit_fields },
    {TZ_OPERATION_TAG_INC_PAID_STG, "Increase paid storage",
     inc_paid_stg_fields                                                             },
    ...
    {TZ_OPERATION_TAG_SORU_EXE_MSG, "SR: execute outbox message",
     soru_exe_msg_fields                                                             },
...
tz_operation_parser_step(tz_parser_state *state)
{
    ...
    PRINTF(
        ...
        (int)regs->ilen, (int)regs->oofs,
        (const char *)PIC(tz_operation_parser_step_name[op->frame->step]),
    ...
    case TZ_OPERATION_STEP_OPERATION: {
        ...
        if (name == NULL) {
            ...
            case TZ_OPERATION_FIELD_SORU_MESSAGES: {
                tz_must(
                    push_frame(state, TZ_OPERATION_STEP_READ_SORU_MESSAGES));
                op->frame->step_read_list.name  = name;
                op->frame->step_read_list.index = 0;
                op->frame->step_read_list.skip  = field->skip;
                tz_must(push_frame(state, TZ_OPERATION_STEP_SIZE));
