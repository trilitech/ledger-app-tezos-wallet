...
tz_err_t handle_unimplemented(__attribute__((unused))command_t *cmd) {
    TZ_PREAMBLE(("cmd=0x%p", cmd));

    PRINTF("[ERROR] Unimplemented instruction 0x%02x\n", cmd->ins);
    TZ_CHECK(EXC_INVALID_INS);
    TZ_POSTAMBLE;
}

tz_err_t handle_apdu_version(__attribute__((unused))command_t *cmd) {
    TZ_PREAMBLE(("cmd=0x%p", cmd));
...
tz_err_t handle_apdu_git(__attribute__((unused))command_t *cmd) {
